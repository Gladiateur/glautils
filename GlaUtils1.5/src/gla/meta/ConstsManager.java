...
public final class ConstsManager implements DefaultConsts {
	...
	public static String constructorStyle;
	...
	private final static Map<String,String> loadConnectionConfig() {
		...
		try {
			...
			/* since 1.4 */
			JOptionPane.showMessageDialog(null, "缺少配置文件或注解：\n"
					+ "\ndefault-connection.propreties：连接数据库相关配置"
					+ "\njavabean.properties:生成实体类相关配置"
					+ "\n解决方案：\n1.使用properties配置文件："
					+ "\n请先执行Config.initConfig() 方法创建配置文件，完成相应配置后再执行本方法！"
					+ "\n\n2.使用注解配置："
					+ "\n在类上声明两个注解：@AutobeansMySQLConfig和@AutobeansEntityConfig"
					+ "\n并根据提示配置相应信息，之后再执行本方法！", 
					"GlaUtils: 生成实体类时发生错误", JOptionPane.ERROR_MESSAGE);
//			throw new RuntimeException(
//					"没有找到配置文件default-connection.propreties");
			throw new RuntimeException();
		}
	...
	private final static Map<String, String> loadBeanConfig() {
		...
		try {
			...
			//标准的javabean中的修饰属性的权限只能值public不能是其他的权限修饰符
			//因此modify这个建没有意义。17.12.21 by Gladiateur
			//modify = prop.getProperty("modify");
			tables = prop.getProperty("tables");
			constructorStyle = prop.getProperty("constructor-style");
			// System.out.println("测试，攻关2：path="+path);
		} catch (IOException e) {
			throw new RuntimeException("读取配置文件异常:" + e.getMessage());
		} catch (NullPointerException e) {
			throw new RuntimeException(
					"没有找到配置文件javabean.propreties");
		}
		Map<String,String> beanMap=new HashMap<String,String>();
		beanMap.put("path", path);
		beanMap.put("checkTableString", checkTableString);
		//beanMap.put("modify", modify);
		beanMap.put("tables", tables);
		beanMap.put("constructor-style", constructorStyle);
		return beanMap;
	}
